...
Groupie is a Shopify rails app that scans customers to see if they have a matching Wikipedia page, IMDB page, or large social following. Wikipedia + IMDB matches are solely based on the customer name, whereas social followings are found by matching customer emails to Fullcontact API.
...
*Wikipedia matches: Groupie checks MediaWiki to see if there is a match with customer name

*IMDB matches: Groupie checks IMDB url to see if there is any returned data for the customers name

*Twitter/YouTube/Klout: Groupie uses FullContact API to get follower counts

*Instagram count: Groupie scrapes the customers Klout url if it exists to find Instagram ID, then pings the Instagram API to get Instagram follower count (not included in Fullcontact response)

* Shopify Customer Webhooks: Whenever a new customer is created in Shopify, the object is sent to Groupie hooks controller for us to scan the customer
